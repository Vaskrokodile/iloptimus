...
import { ArrowRight, Boxes, FlaskConical, Gamepad2, Plus, Sparkles, Trash2 } from "lucide-react";
...
export default function MyEnvironmentsPage() {
  ...
  return <section className="product-page">
    ...
    {environments.length===0?<div className="empty-environments"><span><Sparkles/></span><h2>No environments yet.</h2><p>Build one visually, or load a chat model and type <code>/il</code> or <code>/rl</code> followed by your goal.</p><button onClick={()=>navigate("/pipelines")}>Open designer <ArrowRight/></button></div>:<div className="environment-grid">{environments.map(environment=><article className="environment-card" key={environment.id}><header><span className={`env-mode ${environment.mode.toLowerCase()}`}>{environment.mode}{environment.kind==="state-machine"?" · SIM":""}</span><button onClick={()=>remove(environment)} aria-label={`Delete ${environment.name}`}><Trash2/></button></header><h3>{environment.name}</h3><p>{environment.description}</p><div className="environment-meta"><span>{environment.kind==="state-machine"?"state machine":environment.domain}</span><span>{environment.tasks.length} {environment.kind==="state-machine"?"episodes":"tasks"}</span><span>{environment.interaction.max_steps} step{environment.interaction.max_steps===1?"":"s"}</span></div><div className="reward-mini"><span style={{width:`${environment.reward.correctness*100}%`}}/><span style={{width:`${environment.reward.reasoning*100}%`}}/><span style={{width:`${environment.reward.efficiency*100}%`}}/></div><footer><small>Ready taskset · {environment.taskset_id}</small><div className="environment-actions">{environment.kind==="state-machine"&&<button onClick={()=>navigate(`/environments/${environment.id}/play`)}><Gamepad2/> Test</button>}<button onClick={()=>navigate(`/studio?environment=${environment.taskset_id}`)}><FlaskConical/> Train <ArrowRight/></button></div></footer></article>)}</div>}
  </section>;
}
