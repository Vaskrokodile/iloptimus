...
import { Activity, ArrowRight, Check, Cpu, FlaskConical, FolderOpen, Play, RotateCcw, Sparkles, Terminal } from "lucide-react";
import { createRun, getHardware, getModels, getRun, getRuns, getTasksets, type HardwareInfo, type ModelInfo, type RunState, type TasksetInfo } from "../api/client";

const stages = ["Prepare", "Load model", "Baseline", "Intuition", "Reinforcement", "Evaluate"];
const presets = {
  quick: { sft: 2, grpo: 1, group: 2, benchmark: 1, reasoning: 64, answer: 64, label: "Quick test", time: "~2–5 min" },
  balanced: { sft: 8, grpo: 4, group: 2, benchmark: 4, reasoning: 192, answer: 96, label: "Balanced", time: "~15–30 min" },
  deep: { sft: 20, grpo: 8, group: 4, benchmark: 8, reasoning: 256, answer: 128, label: "Deep train", time: "~45–90 min" },
};
...
export default function OptimusLabPage() {
  ...
  const start = async () => {
    ...
    try {
      const created = await createRun({ model_id: modelId, taskset_id: tasksetId, sft_iters: config.sft, grpo_iters: config.grpo, grpo_group_size: config.group, benchmark_tasks: config.benchmark, max_reasoning_tokens: config.reasoning, max_answer_tokens: config.answer });
