...
from .stateful_environments import scaffold_simulator, stateful_tasks, validate_simulator
...
def validate_environment(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "state-machine":
        raw_simulator = payload.get("simulator") or {}
        if not raw_simulator.get("state"):
            raw_simulator = scaffold_simulator(goal, raw_simulator.get("template_id"))
        simulator = validate_simulator(raw_simulator)
        tasks = [_normalise_task(task, index) for index, task in enumerate(stateful_tasks(simulator))]
    else:
        kind = "task"
    ...
    return {
        ...
        "interaction": {
            ...
            if simulator
            else max(1, min(64, int(payload.get("interaction", {}).get("max_steps", 1)))),
        },
        "reward": {
        ...
        **({"simulator": simulator} if simulator else {}),
        ...
        "template_id": simulator["template_id"] if simulator else f"{mode.lower()}-verifiable-v{FRAMEWORK_VERSION}",
        "builder": {
...
def _package_source(spec: dict[str, Any]) -> str:
    data = repr(spec)
    return f'''"""Generated by the Optimus Studio no-code environment builder."""
from iloptimus.core.environment_framework import score_environment_task
...
class GeneratedTask:
    ...
    def score(self, response: str) -> float:
        return score_environment_task(SPEC, SPEC["tasks"].index(self.data), response)
...
def save_environment(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    (folder / "README.md").write_text(
        f"# {spec['name']}\n\n{spec['description']}\n\n"
        f"Generated as an executable {spec['mode']} environment by Optimus Studio framework v{FRAMEWORK_VERSION}.\n\n"
